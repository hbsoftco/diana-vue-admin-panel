...
Icons are registered explicitly in `src/shared/icons/registry.ts`. The registry separates icons
into two groups generated by `unplugin-icons`:

- Core icons used by the application shell, navigation, or reusable component defaults are
  imported statically and render synchronously.
- Feature-specific and showcase icons retain asynchronous imports and separate build chunks.

The combined `iconRegistry` preserves one set of stable public icon names. Core and lazy entries
...
- Resolves both core and lazy icons without exposing the loading strategy to consumers.
- Renders core icons directly and creates reusable async wrappers for lazy icons.
