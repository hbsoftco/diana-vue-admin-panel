...
<template>
  <div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
    <PreviewCodeCard :title="$t('features.advanced-ui.carousel.basic')" :code="basicCode">
      <DiCarousel
        :slide-count="3"
        :label="$t('features.advanced-ui.carousel.label')"
        :previous-label="$t('features.advanced-ui.carousel.previous')"
        :next-label="$t('features.advanced-ui.carousel.next')"
        :slide-label="$t('features.advanced-ui.carousel.slide')"
      >
        <template #default="{ index }">
          <div
            class="grid min-h-64 w-full place-items-center bg-base-200 text-4xl font-bold text-base-content"
          >
            {{ $t('features.advanced-ui.carousel.slide') }} {{ index + 1 }}
          </div>
        </template>
      </DiCarousel>
    </PreviewCodeCard>

    <PreviewCodeCard :title="$t('features.advanced-ui.carousel.controlled')" :code="controlledCode">
      <DiCarousel
        v-model="activeSlide"
        :slide-count="3"
        loop
        ...
        <template #default="{ index }">
          ...
          >
            <p class="text-sm font-semibold uppercase tracking-wide">
              {{ $t('features.advanced-ui.carousel.featured') }}
            </p>
            <h3 class="mt-2 text-3xl font-bold">
              {{ $t('features.advanced-ui.carousel.story') }} {{ index + 1 }}
            </h3>
          </article>
        </template>
      </DiCarousel>
      <p class="mt-3 text-xs text-base-content/60">
        {{ $t('features.advanced-ui.carousel.active') }}: {{ activeSlide + 1 }}
      </p>
    </PreviewCodeCard>

    <PreviewCodeCard
      class="xl:col-span-2"
      :title="$t('features.advanced-ui.carousel.indicatorsOnly')"
      :code="minimalCode"
    >
      <DiCarousel
        :slide-count="3"
        :show-controls="false"
        :label="$t('features.advanced-ui.carousel.featured')"
        :slide-label="$t('features.advanced-ui.carousel.slide')"
      >
        <template #default="{ index }">
          <div
